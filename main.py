...
class Main1(Screen):
    ...
    def on_enter(self, *args):
        ...
        cur=self.conn.execute("select ID,Timestamp,name,palce,amount,extra,status,balance,total from mdoc")
    ...
    def on_leave(self, *args):
        self.db.close()
        self.ids.tbl.remove_widget(self.data_tables)

    def remove_row(self) -> None:
        if  self.crow[0]:
            x=self.crow[0]
            qry="update mdoc set deadid=0 where ID=?"
            self.conn.execute(qry,(x,))
            self.db.commit()
            data=self.conn.execute('SELECT ID,Timestamp,name,palce,amount,extra,status,balance,total from mdoc where deadid=1')
    ...
    def clear(self) -> None:
        ...
        self.conn.execute("SELECT ID,Timestamp,name,palce,amount,extra,status,balance,total from mdoc where deadid=1")
        for i in self.conn:
            try:
...
class Carddev(Screen):
    ...
    def on_enter(self, *args):
        ...
        self.on_line_chats()
    def on_leave(self, *args):
        self.ids.akbarchat.remove_widget(self.barchart)
        self.ids.akpiechat.remove_widget(self.piechart)
        self.ids.aklinechat.remove_widget(self.linechart)
